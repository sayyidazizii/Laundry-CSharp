...
namespace laundry_1._0
{
    public partial class FrmLaporan : Form
    {
        ...
        //Method untuk menampilkan data dari database ke datagrid
        private void DisplayData()
        {
            var contex = new laundryEntities1();//inisiasi koneksi
            laundryEntities1 laundry = new laundryEntities1(); //koneksi
            var item = laundry.transaksi_masuk.SqlQuery("Select * from transaksi_masuk").ToList();
            if (item != null) //percabangan untuk melihat apakah var item null atau bukan
            {//jika bukan null, maka
                dataGridView1.DataSource = item; //var item adalah datasource dari datagrid 
            }
        }
        //Button batal
        ...
        {
            DisplayData();//memanggil method untuk menampilkan data ke datagridview
        }
        ...
        {
            {
                ...
                try
                {
                    ...
                    {
                        for (int i = 0; i < row.Cells.Count; i++) //perulangan untuk index row
                        {
                            if (row.Cells[i].Value != null && row.Cells[i].Value.ToString().Equals(searchValue))
                            //perulangan untuk index cell
                            {
                    ...
                    if (!valueResult)
                    {
                        MessageBox.Show("Keyword " + TxtCari.Text, "tidak di temukan!");
                        return;
                    }
                }
                catch (Exception exc)
        ...
        {
            searchart();//memanggil method cari
        }
